 ...
 * switching over, never an ephemeral npx cache whose
...
/**
 * Installs `<cli-package>@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  return yield* Effect.gen(function* () {
    const installStep = `installing the pinned ${CLI_PACKAGE_NAME} runtime (this can take a few minutes)`;
    yield* runner
      .run({
        command: "npm",
        args: [
